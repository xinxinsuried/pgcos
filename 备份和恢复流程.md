# 备份和恢复流程
## 拉取
1. docker pull ghcr.io/xinxinsuried/pgcos:latest

## 备份流程（全量逻辑备份）
1. 进入面板：`docker compose run --rm panel`。
2. 选择 configure：填写 `instance_id`（必须手动输入）、PG 容器、超级用户、COS 配置等。
3. 选择 test-connection：校验 PG 容器与 COS 访问。
4. 选择 backup-now：触发一次全量备份。
5. 选择 list：会以表格显示备份时间、年龄、大小与数据库数量；表格后可输入序号或 `backup_id` 查看该备份详情（回车默认最新）。
5. 备份产物会上传到：
   - `s3://{bucket}/pg-backup/{instance_id}/{YYYY-MM-DD_HH-MM-SS}/globals.sql.zst`
   - `s3://{bucket}/pg-backup/{instance_id}/{YYYY-MM-DD_HH-MM-SS}/{dbname}.dump.zst`
   - 对应 `.sha256` 校验文件
   - `s3://{bucket}/pg-backup/{instance_id}/{YYYY-MM-DD_HH-MM-SS}/metadata.json`

`metadata.json` 字段：`instance_id`、`timestamp`、`db_count`、`databases`、`db_sizes`、`total_bytes`。

## 自动定时备份流程
1. `docker compose up -d` 启动 scheduler 服务。
2. scheduler 读取 `./config/config.env` 中的 `SCHEDULE_CRON`（默认每天 03:00）。
3. 如修改配置，请执行 `docker compose restart scheduler` 使其生效。

## 更新镜像
- 面板中选择 update-self，会重新拉取当前镜像。

## 一键测试恢复
- 面板中选择 test-restore，会创建一个临时 PostgreSQL 容器并恢复 latest。
- 默认使用镜像 `postgres:16-alpine`，可通过环境变量 `PG_TEST_IMAGE` 覆盖。
- 恢复完成后会提示临时容器名，可手动删除：`docker rm -f <name>`。

## 一键测试全流程
- 面板中选择 test-flow，会执行：备份 → 恢复 → 自动验证 → 删除临时容器。

## 清理旧备份（prune）
- prune 会按配置的保留天数/保留份数删除旧备份。

## 恢复流程（跨服务器/新容器）
1. 启动一个空的 PostgreSQL 容器（版本必须 >= 源版本）。
2. 在新服务器运行：`docker compose up -d`。
3. 进入面板：`docker compose run --rm panel`。
4. 选择 configure：指向新 PG 容器与同一 COS；填写同一个或要恢复的 `instance_id`。
5. 选择 restore latest 或 restore select：
   - restore latest：恢复该实例的最近一次备份
   - restore select：列出该实例最近 20 个备份供选择
6. 恢复完成后，可使用原有 dbname/user/password 直接连接。

## 重要提示
- 不修改 PostgreSQL 配置；仅使用 `docker exec` 调用 `pg_dump/pg_dumpall/psql/pg_restore`。
- 目标 PostgreSQL 版本必须 >= 源版本。
- 如使用扩展（extension），目标容器需具备对应扩展，否则恢复可能失败。
- `instance_id` 用于多实例区分；列表/恢复/清理会先选择实例。
